...
function my_menu_pages(){
    add_menu_page('CRUD', 'CRUD', 'manage_options', 'view-entries', 'my_submenu_output' );
    add_submenu_page('view-entries', 'CRUD Application', 'View Entries', 'manage_options', 'view-entries', 'my_submenu_output' );
    add_submenu_page('view-entries', 'CRUD Application', 'New Entry', 'manage_options', 'new-entry', 'my_menu_output' );
}
...
class EntryListTable extends WP_List_Table {
    ...
    function prepare_items() {
      ...
      $order = (isset($_REQUEST['order']) && in_array($_REQUEST['order'], array('asc', 'desc'))) ? $_REQUEST['order'] : 'desc';
        ...
        $this->items = $wpdb->get_results($wpdb->prepare("SELECT * FROM $table_name WHERE `title` LIKE '%".$_REQUEST['s']."%'  ORDER BY $orderby $order LIMIT %d OFFSET %d", $per_page, $paged * $per_page), ARRAY_A);
		  } else {
...
?>
  <div class="wrap wqmain_body">
    <h3 class="wp-heading-inline">View Entries</h3>
    <a href="<?php echo admin_url('admin.php?page=new-entry') ?>" class="page-title-action">Add New</a>
